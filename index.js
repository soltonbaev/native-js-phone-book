// ====================DECLARE GLOBALS======================
const API = 'https://soltonbaev.com/json-server/api/contacts';
...
let sort = document.getElementsByClassName('contacts__sort-btn')[0];
let selected;
//==================LAUNCH EVENT LISTENERS=====================

sort.addEventListener('click', async function () {
   let sortedAsc = await getJSON('sortAsc');

   console.log(sortedAsc);
   renderNames(sortedAsc);
});
...
async function renderNames(storage) {
...
async function getJSON(parameter) {
   let contactsResponse = await fetch(
      API +
         `${
            parameter && parameter === 'sortAsc' ? '?_sort=name&_order=asc' : ''
         }`
   );
   result = await contactsResponse.json();

   // let contactsResponse = await fetch(API + "?_sort=name&_order=asc");
   // let result = await contactsResponse.json();
   return result;
}
